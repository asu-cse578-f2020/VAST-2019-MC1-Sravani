...
document.addEventListener('DOMContentLoaded', function () {
  ...
  Promise.all([d3.json("data/StHimartEvents.json")]).then(jsonData => {cityEvents = jsonData[0] })
  ...
  fetch("http://localhost:5000/reportcountanddamage/" + (loc))
    .then((response) => {
      response.json().then(function (data) {
        ...
        let selectedCityEvents = cityEvents.filter(cityEvent => cityEvent.Id == +getClickedCity())[0];
        svg.append("text")
          .attr("x", 150)
          .attr("y", 40)
          .attr("dy", "1em")
